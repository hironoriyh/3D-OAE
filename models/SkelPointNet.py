...
class SkelPointNet(nn.Module):
    ...
    # def compute_loss_chamfer():


    def init_graph(self, shape_xyz, skel_xyz, valid_k=8):

        bn, pn = skel_xyz.size()[0], skel_xyz.size()[1] #bn 1, pn 100

        knn_skel = DF.knn_with_batch(skel_xyz, skel_xyz, pn, is_max=False) #1,100,100
        knn_sp2sk = DF.knn_with_batch(shape_xyz, skel_xyz, 3, is_max=False) #1, 2000, 3 (closest top 3 skelton idicies for each surface point )
        # import ipdb; ipdb.set_trace()

        A = torch.zeros((bn, pn, pn)).float().cuda()

        # initialize A with recovery prior: Mark A[i,j]=1 if (i,j) are two skeletal points closest to a surface point
        A[torch.arange(bn)[:, None], knn_sp2sk[:, :, 0], knn_sp2sk[:, :, 1]] = 1 # giving 2000 indicies for 100 elements, but it works 
    ...
    def forward(self, input_pc, compute_graph=False, recon=False, group=False, device=0):
        ...
        if compute_graph:
            ...
            if group: ## used for point-oae
                ...
                # normalize
                neighborhood = neighborhood - skel_xyz.unsqueeze(2)
                return skel_xyz, skel_r, shape_cmb_features, neighborhood
            else:
